...
1. **Scalars**: `D` (`"days"`) and `skill_levels` — needed to size all subsequent arrays.
2. **shift_types** → `in.shift_types`: reads the `"shift_types"` JSON array and stores it. Immediately builds a `shift_idx` map (`name → int`) so shift strings are resolved to canonical integers everywhere. `shifts_per_day = shift_types.size()`.
3. **age_groups** → `in.age_groups`: reads the `"age_groups"` JSON array and stores it. Immediately builds an `age_group_idx` map (`name → int`). The integer assigned to each age group is its **position in this array** — the same ordering the validator uses for S1. This is the only source of age-group integers; no fallback or first-seen logic is used.
4. **Weights** (`in.w_*`): the 8 soft-constraint multipliers from the `"weights"` JSON object. Always present; JSON is unordered so field-order in the file has no effect.
5. **OTs** (`in.ots[]`): `id` + `availability[]` array parsed directly from `"operating_theaters"`. Always exactly D elements; direct array access, no scalar-broadcast fallback.
6. **Rooms** (`in.rooms[]`): `id` + bed capacity. Immediately builds `room_idx_by_id` (string → int). All downstream references use integer indices only.
7. **Nurses** (`in.nurses[]`): `id` + skill `level` (field `"skill_level"`) + `working_shifts` list (day, `max_load`, shift resolved via `shift_idx` map).
8. **Surgeons** (`in.surgeons[]`): `id` + per-day `max_surgery_time[]`. Builds `surgeon_idx_by_id`.
9. **Patients** (`in.patients[]`): canonical field names — `surgery_release_day` → `release_date`; `surgery_due_day` (mandatory only, default `D−1`) → `due_date`; `surgery_duration` → `surgery_time`; `surgeon_id` → `surgeon_idx`; `incompatible_room_ids` → `incompatible_room_idxs`; `workload_produced` → `nurse_load_per_shift`; `skill_level_required` → `skill_level_required_per_shift`; `gender` → `sex`; `age_group` via `age_group_idx`.
10. **Occupants** (`in.occupants[]`): `room_id` → `room_idx`; `gender`, `workload_produced`, `skill_level_required` (same canonical names); `age_group` via `age_group_idx`.
